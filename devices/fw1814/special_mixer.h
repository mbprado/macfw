...
inline bool applyStraightAnalogPlaybackRouting(FireWireDevice& device,
    ...
    const std::array<UInt32, 2> headphoneGainAddresses{{
        kGainHeadphone1OutLo, kGainHeadphone2OutLo,
    }};
    const std::array<const char*, 2> headphoneGainNames{{
        "GAIN_HP_1_OUT", "GAIN_HP_2_OUT",
    }};
    for (std::size_t output = 0;
         output < headphoneGainAddresses.size(); ++output) {
        if (!writeMixerRegister(
                device, headphoneGainAddresses[output],
                stereoMonitorLevelWord(kMonitorLevelUnity))) {
            if (verbose)
                std::cerr << "FW1814 " << headphoneGainNames[output]
                          << " write failed\n";
            return false;
        }
        if ((*native)->GetBusGeneration(native, &generation) !=
    ...
    if (!writeMixerRegister(device, kGainAnalog56InLo,
                            stereoMonitorLevelWord(kMonitorLevelUnity))) {
        if (verbose) std::cerr << "FW1814 GAIN_ANA_56_IN write failed\n";
        return false;
    }
    ...
    if ((*native)->GetBusGeneration(native, &generation) != kIOReturnSuccess ||
        generation != expectedGeneration) {
        if (verbose)
            std::cerr << "FW1814 generation changed after GAIN_ANA_56_IN; "
                         "stopping routing sequence\n";
        return false;
    }
    ...
    const std::array<UInt32, 4> panAddresses{{
        kLrAnalog12InLo, kLrAnalog34InLo, kLrAnalog56InLo,
        kLrAnalog78InLo,
    }};
    const std::array<const char*, 4> panNames{{
        "LR_ANA_12_IN", "LR_ANA_34_IN", "LR_ANA_56_IN",
        "LR_ANA_78_IN",
    }};
    for (std::size_t pair = 0; pair < panAddresses.size(); ++pair) {
        if (!writeMixerRegister(device, panAddresses[pair],
                                kAnalogInputPanBaseline)) {
            if (verbose)
                std::cerr << "FW1814 " << panNames[pair]
                          << " write failed\n";
            return false;
        }
        if ((*native)->GetBusGeneration(native, &generation) !=
    ...
    if (!writeMixerRegister(device, kSrcHeadphoneOutLo,
                            kHeadphonesFromMixer12)) {
        if (verbose) std::cerr << "FW1814 SRC_HP_OUT write failed\n";
        return false;
    }

    if ((*native)->GetBusGeneration(native, &generation) != kIOReturnSuccess ||
        generation != expectedGeneration) {
        if (verbose) std::cerr << "FW1814 generation changed after SRC_HP_OUT\n";
        return false;
    }
