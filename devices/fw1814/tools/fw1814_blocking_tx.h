...
class BlockingSilenceTransmitRing {
public:
    BlockingSilenceTransmitRing() = default;
    ~BlockingSilenceTransmitRing() { reset(); }

    BlockingSilenceTransmitRing(const BlockingSilenceTransmitRing&) = delete;
    BlockingSilenceTransmitRing& operator=(const BlockingSilenceTransmitRing&) = delete;

    BlockingSilenceTransmitRing(BlockingSilenceTransmitRing&& other) noexcept {
        moveFrom(std::move(other));
    }

    BlockingSilenceTransmitRing& operator=(BlockingSilenceTransmitRing&& other) noexcept {
        if (this != &other) {
            reset();
            moveFrom(std::move(other));
        }
    ...
    static BlockingSilenceTransmitRing create48k(FireWireDevice& device,
                                                   UInt32 firstCycle,
                                                   std::uint8_t dbs,
                                                   std::uint8_t pcmChannels,
                                                   std::size_t packetCount = 128) {
        return create(device, firstCycle, dbs, pcmChannels, 8, 0x02,
                      packetCount);
    }

    static BlockingSilenceTransmitRing create96k(FireWireDevice& device,
                                                   UInt32 firstCycle,
                                                   std::uint8_t dbs,
                                                   std::uint8_t pcmChannels,
                                                   std::size_t packetCount = 128) {
...
private:
    static BlockingSilenceTransmitRing create(FireWireDevice& device,
                                               UInt32 firstCycle,
                                               std::uint8_t dbs,
                                               std::uint8_t pcmChannels,
                                               std::uint8_t events,
        ...
        for (std::size_t i = 0; i < packetCount; ++i) {
            ...
            putBe32(payload,
                    (static_cast<std::uint32_t>(dbs) << 16) | dbc);

            if (phase == 3u) {
                // Blocking-mode empty packet. Keep the current FDF and mark
                // absence of timing information with SYT=0xffff, matching the
                // already hardware-proven macfw blocking transmitter behavior.
                putBe32(payload + 4, 0x90000000u |
                         (static_cast<std::uint32_t>(fdf) << 16) | 0xffffu);
            ...
            putBe32(payload + 4, 0x90000000u |
                    (static_cast<std::uint32_t>(fdf) << 16) | syt);
...
private:
    ...
    static constexpr std::size_t kMaxDbs = 32;
    static constexpr std::size_t kMaxPacketBytes = 8 + 16 * kMaxDbs * 4;
    ...
    void moveFrom(BlockingSilenceTransmitRing&& other) noexcept {
