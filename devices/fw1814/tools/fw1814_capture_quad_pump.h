...
private:
    ...
    std::size_t processChunk(
        ...
        for (std::size_t index = begin; index < end; ++index) {
            ...
            // applying the DBS=3 data-packet validator.
            if (packet.length == 8) {
                if (h.syt == 0xffffu) ++stats_.noDataPackets;
                continue;
            }

            // Quad-rate capture is 2 PCM + 1 MIDI = 3 quadlets per
            // 32-event packet. Report raw PCM positions 0/1 without
            ...
            if (events == 0 || events > 32 || count >= candidates.size()) {
                out.malformedPackets.fetch_add(1, std::memory_order_relaxed);
                continue;
            }
        ...
        // by 32 per packet and repeats after eight packets, so it cannot order
    ...
    std::size_t decode(
        ...
        for (std::size_t event = 0; event < candidate.events; ++event) {
            ...
            for (std::size_t physical = 0;
                 physical < kQuadCapturePcmPositions;
                 ++physical) {
                const std::size_t pos = physical;
                std::int32_t raw = 0;
            ...
            p += kQuadCaptureStreamPositions * 4;
        }
