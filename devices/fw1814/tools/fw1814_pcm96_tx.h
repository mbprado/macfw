...
// Experimental live 96-kHz host->FW1814 transmitter for the hardware-proven S/PDIF-mode
// formation: 6 PCM + 1 MIDI (DBS=7), CIP_BLOCKING, 16/16/16/NODATA cadence.
class BlockingPcmTransmitRing96k {
public:
    struct RefillResult {
    ...
    ~BlockingPcmTransmitRing96k() { reset(); }
    BlockingPcmTransmitRing96k(const BlockingPcmTransmitRing96k&) = delete;
    BlockingPcmTransmitRing96k& operator=(const BlockingPcmTransmitRing96k&) = delete;

    BlockingPcmTransmitRing96k(BlockingPcmTransmitRing96k&& other) noexcept {
        moveFrom(std::move(other));
    }
    BlockingPcmTransmitRing96k& operator=(BlockingPcmTransmitRing96k&& other) noexcept {
        if (this != &other) {
            reset();
            moveFrom(std::move(other));
        }
    ...
    static BlockingPcmTransmitRing96k create(FireWireDevice& device,
                                              UInt32 firstCycle,
                                              std::size_t packetCount = 128) {
        BlockingPcmTransmitRing96k ring;
        ...
        for (std::size_t i = 0; i < packetCount; ++i) {
            ...
            putBe32(payload + 4, 0x90040000u | static_cast<std::uint32_t>(syt));
...
private:
    ...
    static constexpr std::uint32_t kTransferDelayTicks =
        0x2e00u - kTicksPerCycle + (kTicksPerSecond * 16u / 96000u);
    ...
    void moveFrom(BlockingPcmTransmitRing96k&& other) noexcept {
