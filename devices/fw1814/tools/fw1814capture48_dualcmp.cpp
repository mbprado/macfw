...
bool setSignalRate(macfw::FireWireDevice& device,
    ...
    if (ctx.length < 8) {
        std::cout << "        short FCP response: " << ctx.length << " bytes\n";
        return false;
    }
    if (!validControlResponse(ctx.bytes[0])) {
        std::cout << "        unexpected AV/C response code: 0x"
                  << std::hex << static_cast<unsigned>(ctx.bytes[0])
                  << std::dec << '\n';
        return false;
    }
    ...
    if (!bodyOk) {
        std::cout << "        AV/C response body mismatch: ";
        printBytes(ctx.bytes.data(), std::min<std::size_t>(ctx.length, 8));
        std::cout << '\n';
    }
    return bodyOk;
}
...
bool run(bool execute, bool raw) {
...

    {
        ...
        std::cout << "CMP: BOTH oPCR[0] and iPCR[0] connected\n"
                  << "companion TX DMA: intentionally NOT started in this experiment\n";
        ...
        if (!specialStreamKick(device, ctx, raw)) {
            std::cout << "status: FAIL - special stream kick failed\n";
            goto cleanup_stream;
        }
...
int main(int argc, char** argv) {
    ...
    std::cout << "macfw fw1814capture48-dualcmp — dual-CMP startup diagnostic\n\n";
