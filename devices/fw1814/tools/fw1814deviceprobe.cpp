...
bool supported(const Unit& u,std::string& personality){
    ...
    // Only the model-specific bootloader identity is accepted here. FW410
    // also reports the generic "FW Bootloader" product while in bootloader
    // mode, so accepting that name would cross-detect FW410 as FW1814.
    if(u.product=="FW 1814 Bootloader"){
