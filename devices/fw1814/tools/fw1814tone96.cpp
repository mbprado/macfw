...
bool run(unsigned position, bool execute, bool raw) {
    if (position >= 6) {
...

    {
        ...
        auto transmitRing = macfw::fw1814::BlockingSilenceTransmitRing::create96k(
        ...
        std::cout << "*** 500 Hz TONE ACTIVE on PCM position " << position
                  << " for 3 seconds ***\n";
...
void usage(const char* argv0) {
    std::cout << "usage: " << argv0
              << " --position <0..5> [--execute --experimental-high-rate] [--raw]\n";
}
...
int main(int argc, char** argv) {
    ...
    std::cout << "macfw fw1814tone96 — experimental high-rate playback-position diagnostic\n\n";
    return run(position, execute, raw) ? 0 : 1;
}
