...
#include "../tools/fw1814_pcm96_stream.h"
#include "../tools/fw1814_capture96_pump.h"
...
constexpr unsigned kRate = 96000;
...
// Restore the previously working 80-ms TX ring after the 128-packet reserve
// produced broken startup playback. Retain the validated 40-ms refill halves.
...
bool run() {
    ...
    if (playbackShared.ring()->sampleRate.load(std::memory_order_acquire) != kRate) {
        std::cerr << "FW1814 playback shared ring is not set to 96000 Hz\n";
        return false;
    }
...

    {
        ...
        if (rollingTx)
            std::cout << "FW1814 96 kHz rolling TX: "
        ...
        macfw::fw1814::experimental::CapturePump96k capturePump;
        ...
        std::cout << "FW1814 96 kHz analog engine starting\n"
        ...
        bool startupOk = true;
        std::cout << "FW1814 96 kHz TX lead: 4096 cycles; waiting 550 ms\n";
        ...
        if (startupOk) {
            rateAttempted = true;
            std::cout << "FW1814 special stream kick: OUTPUT 96000 Hz\n";
            startupOk = fcp.setSignalRate(kRate, 0x18, false);
        }
        if (startupOk) {
            std::this_thread::sleep_for(std::chrono::milliseconds(100));
            std::cout << "FW1814 special stream kick: INPUT 96000 Hz\n";
        ...
        if (startupOk) {
            ...
            playbackActive = true;
            std::cout << "FW1814 96 kHz analog engine ONLINE\n";
        }
