...
// Experimental 88.2-kHz host->FW1814 transmitter for the hardware-known S/PDIF-mode
// formation: 6 PCM + 1 MIDI (DBS=7), CIP_BLOCKING, sixteen events in each
...
//
// This 640-packet ring follows 88.2 kHz / 16 events / 8000 cycles, which reduces to
...
class BlockingPcmTransmitRing88200 {
public:
    struct RefillResult {
    ...
    ~BlockingPcmTransmitRing88200() { reset(); }
    BlockingPcmTransmitRing88200(const BlockingPcmTransmitRing88200&) = delete;
    BlockingPcmTransmitRing88200& operator=(const BlockingPcmTransmitRing88200&) = delete;

    BlockingPcmTransmitRing88200(BlockingPcmTransmitRing88200&& other) noexcept {
        moveFrom(std::move(other));
    }
    BlockingPcmTransmitRing88200& operator=(BlockingPcmTransmitRing88200&& other) noexcept {
        if (this != &other) {
            reset();
            moveFrom(std::move(other));
        }
    ...
    static BlockingPcmTransmitRing88200 create(FireWireDevice& device,
                                                UInt32 firstCycle,
    ...
    // Advance the native 88.2 kHz DBC/SYT cadence without touching live DMA slots.
    void advanceState(macfw::am824::Playback44100State& state,
...
private:
    ...
    static PacketImage buildSilencePacket(
        ...
        if (!packet.dataBearing) {
            // AM824 FDF rate code 3 = 88.2 kHz; no SYT on NODATA.
            putBe32(packet.bytes + 4, 0x9003ffffu);
            packet.length = 8;
            return packet;
        }
        ...
        putBe32(packet.bytes + 4,
                0x90030000u | static_cast<std::uint32_t>(syt));
    ...
    void moveFrom(BlockingPcmTransmitRing88200&& other) noexcept {
...
public:
    ...
    BlockingPcmStream88200(BlockingPcmTransmitRing88200& tx,
...
private:
    ...
    BlockingPcmTransmitRing88200* tx_ = nullptr;
