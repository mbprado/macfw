...
private:
    ...
    void handleInputMonitorLevel(const std::string& command) {
        ...
        const std::string setAllPrefix = "INPUT_MONITOR_LEVEL SET_ALL ";
        const std::string setPrefix = "INPUT_MONITOR_LEVEL SET ";
        const bool getting = command.rfind(getPrefix, 0) == 0;
        ...
        if (getting) {
            ...
            const int cachedLeft = macfw::fw1814::monitorLevelRaw(
                macfw::fw1814::monitorLevelChannel(gainAnalogIn_[pair], 0));
            const int cachedRight = macfw::fw1814::monitorLevelRaw(
                macfw::fw1814::monitorLevelChannel(gainAnalogIn_[pair], 1));
            reply("OK " + std::to_string(pair) + " " +
        ...
        if (settingAll) {
            ...
            desired = macfw::fw1814::stereoMonitorLevelWord(levels[level]);
        } else {
        ...
        const int returnedLeft = macfw::fw1814::monitorLevelRaw(
            macfw::fw1814::monitorLevelChannel(gainAnalogIn_[pair], 0));
        const int returnedRight = macfw::fw1814::monitorLevelRaw(
            macfw::fw1814::monitorLevelChannel(gainAnalogIn_[pair], 1));
        reply("OK " + std::to_string(pair) + " " +
              std::to_string(returnedLeft) + " " +
              std::to_string(returnedRight) + " " +
              hex32(gainAnalogIn_[pair]) + "\n");
    }
    ...
    void handleAnalogOutputLevel(const std::string& command) {
        ...
        if (!analogOutputLevelKnown_[pair]) {
            reply("ERR analog-output-level-state-uninitialized\n");
            return;
        }
    ...
    void handleHeadphoneOutputLevel(const std::string& command) {
        ...
        const std::string setPrefix = "HEADPHONE_OUTPUT_LEVEL SET ";
        const bool getting = command.rfind(getPrefix, 0) == 0;
        const bool settingAll = command.rfind(setAllPrefix, 0) == 0;
        const bool setting = command.rfind(setPrefix, 0) == 0;
        if (!getting && !settingAll && !setting) {
        ...
        const int cachedLeft = macfw::fw1814::monitorLevelRaw(
            macfw::fw1814::monitorLevelChannel(
                gainHeadphoneOut_[output], 0));
        const int cachedRight = macfw::fw1814::monitorLevelRaw(
            macfw::fw1814::monitorLevelChannel(
                gainHeadphoneOut_[output], 1));
        reply("OK " + std::to_string(output) + " " +
              std::to_string(cachedLeft) + " " +
              std::to_string(cachedRight) + " " +
    ...
    void handleAuxAnalogInputLevel(const std::string& command) {
        ...
        if (!auxAnalogInputLevelKnown_[source]) {
            reply("ERR aux-analog-input-level-state-uninitialized\n");
            return;
        }
        ...
        if (settingAll || setting) {
            ...
            const std::array<UInt32, 4> addresses{{
                ...
                macfw::fw1814::kGainAuxAnalog56InLo,
                macfw::fw1814::kGainAuxAnalog78InLo,
            }};
            const WriteResult result =
                writeRegister(addresses[source], desired);
            if (result != WriteResult::Ok) {
            ...
            gainAuxAnalogIn_[source] = desired;
        }

        const int cachedLeft = macfw::fw1814::monitorLevelRaw(
            macfw::fw1814::monitorLevelChannel(gainAuxAnalogIn_[source], 0));
        const int cachedRight = macfw::fw1814::monitorLevelRaw(
            macfw::fw1814::monitorLevelChannel(gainAuxAnalogIn_[source], 1));
        reply("OK " + std::to_string(source) + " " +
              std::to_string(cachedLeft) + " " +
              std::to_string(cachedRight) + " " +
    ...
    void handleInputMonitorChannelLevel(const std::string& command) {
        ...
        if (getting) {
            ...
            reply("OK " + std::to_string(pair) + " " +
                  ...
                  std::to_string(macfw::fw1814::monitorLevelRaw(cached)) +
                  " " +
                  hex32(gainAnalogIn_[pair]) + "\n");
            return;
        }
        ...
        reply("OK " + std::to_string(pair) + " " +
              std::to_string(channel) + " " +
              std::to_string(macfw::fw1814::monitorLevelRaw(channelLevel)) +
              " " +
              hex32(gainAnalogIn_[pair]) + "\n");
    }
    ...
    void handle(const std::string& wireCommand) {
        ...
        if (command.rfind("INPUT_MONITOR_PAN ", 0) == 0) {
        ...
        if (command.rfind("ANALOG_OUTPUT_LEVEL ", 0) == 0) {
        ...
        if (command.rfind("AUX_ANALOG_INPUT_LEVEL ", 0) == 0) {
    ...
    std::array<bool, 4> analogInputPanKnown_{{
        false, false, false, false,
    }};
    std::array<std::uint32_t, 4> panAnalogIn_{{0, 0, 0, 0}};
    int listenFd_ = -1;
    int clientFd_ = -1;
    std::string request_;
};
