...
#include "../common/am824_playback.h"
...
constexpr size_t kPlaybackBufferBytes =
    macfw::am824::kPlayback48kDataPacketBytes;
...
struct PlaybackSlot {
    UInt32 status;
    UInt32 timestamp;
    UInt8 payload[kPlaybackBufferBytes];
};

struct TxDclContext {
    IOFireWireLibNuDCLPoolRef pool = nullptr;
    IOFireWireLibLocalIsochPortRef *localPort = nullptr;
    PlaybackSlot *slot = nullptr;
    std::size_t slotIndex = 0;
    std::uint8_t nextDbc = 0;
    std::uint64_t callbacks = 0;
    std::uint64_t updates = 0;
    std::uint64_t zeroTimestamps = 0;
...
static std::uint8_t playbackDbcForSlot(std::size_t slot) {
    const std::size_t nodataBefore = slot / 4;
    const std::size_t dataBefore = slot - nodataBefore;
    return static_cast<std::uint8_t>((dataBefore * 8) & 0xff);
}

static void playbackDclComplete(void *refcon, NuDCLRef dcl) {
    auto *ctx = static_cast<TxDclContext *>(refcon);
    if (!ctx || !ctx->pool || !ctx->localPort ||
        !*ctx->localPort || !ctx->slot) {
        return;
    }

    ++ctx->callbacks;

    const UInt32 timestamp = ctx->slot->timestamp;
    if (timestamp == 0) {
        ++ctx->zeroTimestamps;
        return;
    }

    const std::uint32_t cycle = (timestamp >> 12) & 0x1fffu;

    macfw::am824::Playback48kState state;
    state.dbc = ctx->nextDbc;
    state.phase = static_cast<std::uint8_t>(ctx->slotIndex & 3u);

    const auto packet =
        macfw::am824::buildPlayback48kSilence(cycle, state);

    std::memcpy(
        ctx->slot->payload, packet.bytes.data(), packet.length);

    IOVirtualRange range = {
        reinterpret_cast<IOVirtualAddress>(ctx->slot->payload),
        packet.length
    };

    if ((*ctx->pool)->SetDCLRanges(
            dcl, 1, &range) != kIOReturnSuccess) {
        ++ctx->updateFailures;
        return;
    }

    void *changed[1] = {reinterpret_cast<void *>(dcl)};
    if ((**ctx->localPort)->Notify(
            *ctx->localPort,
    ...
    // One 64-cycle ring contains 48 data packets.  At 8 data
    // blocks each, DBC advances 384 == 128 modulo 256 per ring.
    ctx->nextDbc =
        static_cast<std::uint8_t>(ctx->nextDbc + 128u);
}
...
static bool run(
...

    {
        ...
        for (const auto &ctx : txContexts) {
            callbacks += ctx.callbacks;
            updates += ctx.updates;
            zeroTimestamps += ctx.zeroTimestamps;
            updateFailures += ctx.updateFailures;
        }

        std::cout << "\nplayback TX timing/update statistics:\n";
        std::cout << "    DCL callbacks:   " << callbacks << '\n';
        std::cout << "    dynamic updates: " << updates << '\n';
        std::cout << "    zero timestamps: " << zeroTimestamps << '\n';
        std::cout << "    update failures: " << updateFailures << '\n';

        std::size_t shown = 0;
        for (std::size_t i = 0;
             i < kPlaybackSlots && shown < 8; ++i) {
            if (playback[i].timestamp == 0)
                continue;

            const std::uint32_t q0 = be32(playback[i].payload);
            const std::uint32_t q1 = be32(playback[i].payload + 4);
            std::cout
                << "    slot " << i
                << ": timestamp=0x" << std::hex
                << playback[i].timestamp
                << " dbs=" << ((q0 >> 16) & 0xff)
                << " dbc=" << (q0 & 0xff)
                << " fdf=0x" << ((q1 >> 16) & 0xff)
                << " syt=0x" << (q1 & 0xffff)
                << std::dec
                << ((i & 3u) == 3u ? " NODATA" : " SILENCE")
                << '\n';
            ++shown;
        }
    }
