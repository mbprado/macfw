...
The important change from earlier development is that the startup lead/reassert delay is no longer an unserviced window. RX publication, playback pumping and TX refill continue during this interval so the transport does not deplete its live rings before READY.
...
The 44.1 value is historical and was explicitly restored after earlier refactoring. Because the current 44.1 path is now hardware-stable with excellent latency, do not reduce this value casually before release.

## Real-time scheduling after READY

Both native engines now use the same validated service architecture:
...
That forced re-arm did **not** reliably solve the actual remaining symptom and added unnecessary startup delay, so it was reverted.
...
Verbose diagnostics are not performance-neutral because reporting occurs on the real-time audio service path. Therefore normal launchd service operation is the release reference; foreground verbose behavior is a deferred diagnostic concern, not justification for another forced rate cycle.

## 48 -> 44.1 switching is slower
...
The slow direction completes correctly and audio returns correctly, so it is a known release non-blocker. Optimize it after release with explicit stage timing rather than removing established 44.1 startup behavior blindly.
...
This policy preserves the now-proven low-latency scheduler at both rates while keeping the FW410-specific 44.1 startup ritual isolated from ordinary 48 kHz operation.
