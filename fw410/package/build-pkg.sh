...
for file in "${required[@]}"; do
    if [[ ! -e "$file" ]]; then
        ...
        echo "build the FW410 HAL and tools first" >&2
        exit 1
    fi
done
...
# Package only the runtime files proven by the launchd validation rather than
...
# productbuild cannot run a preinstall helper from the payload before the
# payload exists. Stage deviceprobe in a tiny prerequisite component whose
# postinstall is the actual hardware gate for the main package transaction.
...
echo "build: $GIT_SHA"
echo
echo "First-package validation target:"
echo "  sudo installer -pkg '$PKG' -target /"
echo "  tools/transport/transportstatus/transportstatus --watch"
